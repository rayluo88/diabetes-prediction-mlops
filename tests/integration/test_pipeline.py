...
from data.preprocess import preprocess_diabetes_data, DiabetesPreprocessor
...
class TestEndToEndPipeline:
    ...
    def test_training_pipeline(self, mock_client_class, mock_log_model, mock_log_metric,
                              mock_log_params, mock_start_run, mock_set_uri, 
                              mock_set_experiment, temp_workspace):
    ...
    def test_prediction_pipeline(self, temp_workspace):
        ...
        assert all(0 <= prob <= 1 for prob_row in prediction_probas for prob in prob_row)
...
class TestPipelineDataFlow:
    ...
    def test_data_consistency_through_pipeline(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            processed_target_dist = pd.concat([
                train_processed['target'], 
                test_processed['target']
            ]).value_counts(normalize=True)
            
            # Allow for small differences due to processing
            for class_label in [0, 1]:
                assert abs(original_target_dist[class_label] - processed_target_dist[class_label]) < 0.05
    ...
    def test_pipeline_reproducibility(self):
        ...
        assert abs(results1[0]['train_target_mean'] - results2[0]['train_target_mean']) < 0.01
        assert abs(results1[0]['test_target_mean'] - results2[0]['test_target_mean']) < 0.01
