...
class DiabetesPreprocessor:
    ...
    def clean_data(self, df):
        ...
        zero_invalid_features = ['glucose', 'blood_pressure', 'skin_thickness', 'insulin', 'bmi']
        ...
        for feature, count in missing_counts.items():
            if count > 0:
                logger.info(f"Missing values in {feature}: {count} ({count/len(df_clean)*100:.1f}%)")
    ...
    def engineer_features(self, df):
        ...
        df_engineered['glucose_bmi_ratio'] = df_engineered['glucose'] / (df_engineered['bmi'] + 1e-8)
        
        # Pregnancy risk (for females)
        df_engineered['high_pregnancy_risk'] = (df_engineered['pregnancies'] > 5).astype(int)
        
        # Convert categorical features to dummy variables
        categorical_features = ['bmi_category', 'age_group', 'glucose_category']
        df_engineered = pd.get_dummies(df_engineered, columns=categorical_features, prefix=categorical_features)
...
if __name__ == "__main__":
    ...
    if os.path.exists(train_path) and os.path.exists(test_path):
        train_processed, test_processed, preprocessor = preprocess_diabetes_data(train_path, test_path)
