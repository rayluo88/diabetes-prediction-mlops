...
async def predict_diabetes(
    ...
    try:
        ...
        if diabetes_probability < 0.3:
            ...
            recommendation = "Lifestyle modifications recommended. Consult healthcare provider."
        else:
        ...
        logger.info(f"Prediction completed - Risk: {risk_category}, Probability: {diabetes_probability:.3f}")
        return result
...
async def predict_batch(
    ...
    try:
        ...
        # Process results
        results = []
        for i, (patient, pred, proba) in enumerate(zip(request.patients, predictions, prediction_probas)):
        ...
        summary = {
            ...
            "average_diabetes_probability": np.mean([r.diabetes_probability for r in results]),
            "processing_time_seconds": (datetime.now() - datetime.now()).total_seconds()
        }
        ...
        logger.info(f"Batch prediction completed - {summary['predicted_diabetic']}/{summary['total_patients']} predicted diabetic")
        return response
