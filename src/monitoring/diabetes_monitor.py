...
class DiabetesModelMonitor:
    ...
    def generate_data_drift_report(self, current_data: pd.DataFrame) -> Dict:
        ...
        report_path = os.path.join(self.monitoring_reports_dir, f"data_drift_report_{timestamp}.html")
    ...
    def generate_model_performance_report(self, 
        ...
        # Save report
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join(self.monitoring_reports_dir, f"performance_report_{timestamp}.html")
    ...
    def run_data_quality_tests(self, current_data: pd.DataFrame) -> Dict:
        ...
        # Save test results
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        test_path = os.path.join(self.monitoring_reports_dir, f"data_quality_tests_{timestamp}.html")
    ...
    def detect_bias_and_fairness(self, 
                                current_data: pd.DataFrame,
                                predictions: np.ndarray,
                                sensitive_features: List[str] = ['age']) -> Dict:
        """
        ...
        for feature in sensitive_features:
            if feature in current_data.columns:
                ...
                if feature == 'age':
                    ...
                    group_stats = current_data.groupby('age_group').agg({
                        'target': ['count', 'mean']
                    }).round(3)
                    
                    prediction_rates = pd.Series(predictions).groupby(
    ...
    def _calculate_demographic_parity(self, groups: pd.Series, predictions: np.ndarray) -> float:
    ...
    def comprehensive_monitoring_check(self, 
                                     current_data: pd.DataFrame,
        ...
        try:
            ...
            if quality_results.get('failed_tests', 0) > 0:
                ...
                monitoring_results['alerts'] = monitoring_results.get('alerts', [])
                monitoring_results['alerts'].append(f"Data quality tests failed: {quality_results['failed_tests']}")
        ...
        if predictions is not None:
            try:
                performance_results = self.generate_model_performance_report(current_data, predictions)
                monitoring_results['model_performance'] = performance_results
                
                # Check performance degradation
                if performance_results.get('accuracy', 1.0) < 0.75:  # Singapore healthcare threshold
                    monitoring_results['monitoring_status'] = 'critical'
                    monitoring_results['alerts'] = monitoring_results.get('alerts', [])
                    monitoring_results['alerts'].append('Model performance below threshold')
            ...
            try:
                ...
                for feature, analysis in bias_results.items():
                    if analysis.get('demographic_parity_difference', 0) > 0.1:
                        monitoring_results['monitoring_status'] = 'warning'
                        monitoring_results['alerts'] = monitoring_results.get('alerts', [])
                        monitoring_results['alerts'].append(f'Bias detected in {feature}')
        ...
        logger.info(f"Comprehensive monitoring completed - Status: {monitoring_results['monitoring_status']}")
        return monitoring_results
    
    def _save_monitoring_summary(self, results: Dict):
        """Save monitoring summary to file."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary_path = os.path.join(self.monitoring_reports_dir, f"monitoring_summary_{timestamp}.json")
...
def load_model_and_predict(data: pd.DataFrame) -> np.ndarray:
    ...
    try:
        ...
        preprocessor = DiabetesPreprocessor.load("models/artifacts/diabetes_preprocessor.pkl")
        
        # Preprocess and predict
        X_processed = preprocessor.transform(data.drop('target', axis=1, errors='ignore'))
...
def main():
    ...
    if 'data_quality' in results:
        ...
        print(f"Quality Tests: {quality.get('passed_tests', 0)}/{quality.get('total_tests', 0)} passed")
