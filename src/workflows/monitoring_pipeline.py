...
def load_monitoring_data_task(
    data_path: str = "data/processed/diabetes_test_processed.csv",
):
...
def run_monitoring_analysis_task(
    monitoring_data: pd.DataFrame, predictions: np.ndarray
) -> Dict:
    ...
    try:
        ...
        if len(predictions) > 0:
            results = monitor.comprehensive_monitoring_check(
        ...
        logger.info(
            f"Monitoring analysis completed - Status: {results['monitoring_status']}"
        )
        return results
...
def evaluate_retraining_criteria_task(monitoring_results: Dict) -> Dict:
    ...
    if quality.get("failed_tests", 0) > 2:
        retraining_decision["retrain_needed"] = True
        retraining_decision["retrain_reasons"].append(
            "Multiple data quality test failures"
        )
        if retraining_decision["priority"] == "low":
            retraining_decision["priority"] = "medium"
...
def trigger_retraining_task(retraining_decision: Dict):
    ...
    try:
        logger.info(
            f"Triggering automated retraining - Priority: {retraining_decision['priority']}"
        )
        ...
        # High/Medium priority: scheduled retraining
            ...
            logger.info(
                f"Scheduling retraining for {retraining_decision['priority']} priority issues"
            )
...
def send_alerts_task(monitoring_results: Dict, retraining_results: Dict):
    ...
    try:
        ...
        alert_summary = {
            ...
            "alerts": alerts,
            "retraining_triggered": retraining_results.get(
        ...
        if retraining_results.get("retraining_triggered"):
            logger.info(
                f"🔄 Model retraining triggered - Priority: {retraining_results.get('priority')}"
            )
...
def generate_dashboard_task(monitoring_results: Dict):
    ...
    try:
        ...
        dashboard_data = {
            ...
            "metrics": {
                "data_drift_share": monitoring_results.get("data_drift", {}).get(
                    "drift_share", 0
                ),
                "data_quality_score": (
                    monitoring_results.get("data_quality", {}).get("passed_tests", 0)
                    / max(
                        monitoring_results.get("data_quality", {}).get(
                            "total_tests", 1
                        ),
                        1,
                    )
                ),
                "model_accuracy": monitoring_results.get("model_performance", {}).get(
                    "accuracy", 0
                ),
                "bias_max_difference": max(
                    [
                        analysis.get("demographic_parity_difference", 0)
                        for analysis in monitoring_results.get(
                            "bias_analysis", {}
                        ).values()
                    ]
                    or [0]
        ...
        if dashboard_data["metrics"]["data_drift_share"] > 0.2:
            dashboard_data["recommendations"].append(
                "Monitor data sources for distribution changes"
            )
        ...
        if dashboard_data["metrics"]["bias_max_difference"] > 0.1:
            dashboard_data["recommendations"].append(
                "Review model for fairness across demographic groups"
            )
...
def create_monitoring_deployment():
    ...
    deployment = Deployment.build_from_flow(
        ...
        parameters={"data_path": "data/processed/diabetes_test_processed.csv"},
