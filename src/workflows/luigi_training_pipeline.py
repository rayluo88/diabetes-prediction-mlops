...
class PreprocessDataTask(luigi.Task):
    ...
    def run(self):
        ...
        try:
            ...
            processed_train.to_csv('data/processed/diabetes_train_processed.csv', index=False)
            processed_test.to_csv('data/processed/diabetes_test_processed.csv', index=False)
...
class TrainModelsTask(luigi.Task):
    ...
    def run(self):
        ...
        try:
            ...
            best_model, best_metrics = trainer.train_optimized_model(X_train, y_train, X_test, y_test)
            
            # Register champion model
            logger.info("Registering champion model...")
            trainer.register_champion_model(best_model, "diabetes_prediction_champion", best_metrics)
    ...
    def run(self):
        ...
        try:
            ...
            with self.output().open('w') as f:
                ...
                f.write(f"Healthcare threshold (>={healthcare_threshold}): {'PASSED' if validation_passed else 'FAILED'}\n")
                
            if not validation_passed:
                raise ValueError(f"Model accuracy {accuracy:.4f} below healthcare threshold {healthcare_threshold}")
