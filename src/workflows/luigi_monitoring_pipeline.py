...
from monitoring.diabetes_monitor import DiabetesModelMonitor  # noqa: E402
...
class RunMonitoringAnalysisTask(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            f"models/monitoring/monitoring_results_{self.date}.json"
        )
    ...
    def run(self):
        ...
        try:
            ...
            bias_report = monitor.detect_bias_and_fairness(
                current_data,
            ...
            monitoring_results = {
                ...
                "summary": {
                    ...
                    "performance_degraded": performance_report.get("accuracy", 1.0)
                    < 0.75,
                    "bias_detected": any(
                        result.get("demographic_parity_difference", 0) > 0.15
                        for result in bias_report.values()
...
class EvaluateRetrainingCriteriaTask(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            f"models/monitoring/retraining_decision_{self.date}.json"
        )
        ...
        try:
            ...
            if performance_degraded:
                ...
                retraining_decision["reasons"].append(
                    "Model performance below healthcare threshold"
                )
                retraining_decision["priority"] = "critical"

            retraining_decision["criteria_evaluation"][
            ...
            if retraining_decision["retrain_needed"]:
                logger.warning(
                    f"⚠️  Retraining needed! Priority: {retraining_decision['priority']}"
                )
                logger.warning(f"Reasons: {', '.join(retraining_decision['reasons'])}")
            else:
                logger.info(
                    "✅ No retraining needed - model performance within acceptable limits"
                )
...
MLflow: http://localhost:5000
                ...
                alert_log.append(
                    f"[{datetime.now()}] No alerts needed - system healthy"
                )
...
class ConditionalRetrainingTask(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            f"models/monitoring/retraining_triggered_{self.date}.flag"
        )
...
4. Update monitoring baseline
                ...
                with self.output().open("w") as f:
                    f.write(
                        f"No retraining needed on {self.date} - model performance acceptable\n"
                    )
...
class DiabetesMonitoringPipeline(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            f"models/monitoring/monitoring_pipeline_complete_{self.date}.flag"
        )
