...
def validate_input_data_task(data_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        for feature, (min_val, max_val) in outlier_checks.items():
            ...
            if outliers.any():
                logger.warning(
                    f"Outliers detected in {feature}: {outliers.sum()} values"
                )
...
def generate_predictions_task(model, preprocessor, input_data: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        return {"predictions_df": results_df, "summary": summary}
